export default {
  common: {
    confirm: '确认',
    cancel: '取消',
    save: '保存',
    delete: '删除',
    edit: '编辑',
    add: '添加',
    create: '创建',
    update: '更新',
    refresh: '刷新',
    search: '搜索',
    filter: '筛选',
    export: '导出',
    import: '导入',
    loading: '加载中...',
    success: '成功',
    error: '错误',
    warning: '警告',
    info: '信息',
    yes: '是',
    no: '否',
    submit: '提交',
    reset: '重置',
    close: '关闭',
    back: '返回',
    next: '下一步',
    previous: '上一步',
    finish: '完成',
    actions: '操作',
    status: '状态',
    name: '名称',
    type: '类型',
    description: '描述',
    createdAt: '创建时间',
    updatedAt: '更新时间',
    noData: '暂无数据',
    total: '总计',
    selected: '已选择',
    all: '全部'
  },
  
  nav: {
    title: '备份服务',
    dashboard: '仪表板',
    tasks: '备份任务',
    logs: '备份日志',
    storages: '存储库管理',
    notifications: '通知管理',
    statistics: '统计信息',
    settings: '设置',
    logout: '退出登录'
  },
  
  auth: {
    login: '登录',
    register: '注册',
    logout: '退出',
    username: '用户名',
    password: '密码',
    confirmPassword: '确认密码',
    loginTitle: '登录到备份服务',
    registerTitle: '注册备份服务账户',
    loginButton: '登录',
    registerButton: '注册',
    forgotPassword: '忘记密码？',
    noAccount: '还没有账户？',
    hasAccount: '已有账户？',
    loginNow: '立即登录',
    registerNow: '立即注册',
    loginSuccess: '登录成功',
    registerSuccess: '注册成功',
    logoutSuccess: '退出成功',
    loginFailed: '登录失败',
    registerFailed: '注册失败',
    invalidCredentials: '用户名或密码错误',
    usernameRequired: '请输入用户名',
    passwordRequired: '请输入密码',
    confirmPasswordRequired: '请确认密码',
    passwordMismatch: '两次输入的密码不一致',
    usernameMinLength: '用户名至少需要3个字符',
    passwordMinLength: '密码至少需要6个字符',
    changePassword: '修改密码',
    currentPassword: '当前密码',
    newPassword: '新密码',
    changePasswordSuccess: '密码修改成功',
    changePasswordFailed: '密码修改失败'
  },
  
  dashboard: {
    title: '仪表板',
    subtitle: '监控您的备份任务和系统状态',
    overview: '概览',
    totalTasks: '总任务数',
    activeTasks: '活跃任务',
    runningBackups: '运行中备份',
    successRate: '成功率',
    recentLogs: '最近日志',
    nextRuns: '下次运行',
    systemStatus: '系统状态',
    systemHealth: '系统健康',
    quickActions: '快速操作',
    autoRefresh: '自动刷新',
    refreshInterval: '刷新间隔',
    lastUpdated: '最后更新',
    noRecentLogs: '暂无最近日志',
    noScheduledRuns: '暂无计划运行',
    healthCheck: '健康检查',
    cleanup: '清理日志',
    exportData: '导出数据',
    backupTrends: '备份趋势',
    systemMetrics: '系统指标',
    cpuUsage: 'CPU 使用率',
    memoryUsage: '内存使用率',
    diskUsage: '磁盘使用率',
    uptime: '运行时间',
    healthy: '健康',
    degraded: '降级',
    unhealthy: '不健康',
    refreshSuccess: '刷新成功',
    refreshFailed: '刷新失败',
    cleanupSuccess: '清理成功',
    cleanupFailed: '清理失败',
    exportSuccess: '导出成功',
    exportFailed: '导出失败'
  },
  
  logs: {
    title: '备份日志',
    taskName: '任务名称',
    status: '状态',
    startedAt: '开始时间',
    completedAt: '完成时间',
    duration: '持续时间',
    message: '消息',
    errorDetails: '错误详情',
    dataSize: '数据大小',
    viewDetails: '查看详情',
    deleteLog: '删除日志',
    confirmDelete: '确认删除此日志？',
    deleteSuccess: '日志删除成功',
    deleteFailed: '日志删除失败',
    noLogs: '暂无备份日志',
    filterByStatus: '按状态筛选',
    filterByType: '按类型筛选',
    filterByDate: '按日期筛选',
    today: '今天',
    yesterday: '昨天',
    thisWeek: '本周',
    thisMonth: '本月',
    lastMonth: '上月'
  },
  
  notifications: {
    title: '通知管理',
    modules: '通知模块',
    preferences: '通知偏好',
    createModule: '创建模块',
    editModule: '编辑模块',
    moduleName: '模块名称',
    moduleType: '模块类型',
    config: '配置',
    isActive: '是否启用',
    testModule: '测试模块',
    deleteModule: '删除模块',
    confirmDelete: '确认删除此模块？',
    createSuccess: '模块创建成功',
    createFailed: '模块创建失败',
    updateSuccess: '模块更新成功',
    updateFailed: '模块更新失败',
    deleteSuccess: '模块删除成功',
    deleteFailed: '模块删除失败',
    testSuccess: '测试成功',
    testFailed: '测试失败',
    noModules: '暂无通知模块',
    wechat: '微信',
    dingtalk: '钉钉',
    webhook: 'Webhook',
    synologyChat: 'Synology Chat'
  },
  
  settings: {
    title: '设置',
    userSettings: '用户设置',
    systemSettings: '系统设置',
    appearance: '外观',
    language: '语言',
    theme: '主题',
    lightTheme: '浅色主题',
    darkTheme: '深色主题',
    autoTheme: '跟随系统',
    chinese: '简体中文',
    english: 'English',
    autoRefresh: '自动刷新',
    refreshInterval: '刷新间隔',
    seconds: '秒',
    saveSuccess: '设置保存成功',
    saveFailed: '设置保存失败',
    resetSettings: '重置设置',
    confirmReset: '确认重置所有设置？'
  },
  
  validation: {
    required: '此字段为必填项',
    minLength: '最少需要 {min} 个字符',
    maxLength: '最多允许 {max} 个字符',
    email: '请输入有效的邮箱地址',
    url: '请输入有效的URL',
    number: '请输入有效的数字',
    integer: '请输入有效的整数',
    positive: '请输入正数',
    range: '请输入 {min} 到 {max} 之间的值'
  },
  
  pagination: {
    total: '共 {total} 条',
    page: '第 {page} 页',
    pageSize: '每页 {size} 条',
    goto: '跳至',
    prev: '上一页',
    next: '下一页',
    first: '首页',
    last: '末页'
  },

  tasks: {
    title: '备份任务',
    createTask: '创建任务',
    editTask: '编辑任务',
    deleteTask: '删除任务',
    runTask: '立即运行',
    toggleTask: '切换状态',
    taskName: '任务名称',
    taskType: '任务类型',
    taskStatus: '任务状态',
    taskSchedule: '执行计划',
    lastRun: '上次运行',
    nextRun: '下次运行',
    sourceConfig: '源配置',
    destinationConfig: '目标配置',
    active: '活跃',
    inactive: '非活跃',
    running: '运行中',
    success: '成功',
    failed: '失败',
    pending: '等待中',
    cancelled: '已取消',

    // Task types
    mysqlToMysql: 'MySQL 到 MySQL',
    mysqlToSmb: 'MySQL 到 SMB',
    minioToMinio: 'MinIO 到 MinIO',

    // Form labels
    name: '任务名称',
    type: '备份类型',
    schedule: '执行计划',
    description: '任务描述',

    // MySQL config
    mysqlHost: 'MySQL 主机',
    mysqlPort: 'MySQL 端口',
    mysqlUsername: 'MySQL 用户名',
    mysqlPassword: 'MySQL 密码',
    mysqlDatabase: 'MySQL 数据库',

    // SMB config
    smbHost: 'SMB 主机',
    smbPort: 'SMB 端口',
    smbUsername: 'SMB 用户名',
    smbPassword: 'SMB 密码',
    smbDomain: 'SMB 域',
    smbDomainPlaceholder: '可选，Windows 域名',
    smbShare: 'SMB 共享',
    smbPath: 'SMB 路径',

    // MinIO config
    minioEndpoint: 'MinIO 端点',
    minioPort: 'MinIO 端口',
    minioAccessKey: 'MinIO 访问密钥',
    minioSecretKey: 'MinIO 密钥',
    minioBucket: 'MinIO 存储桶',
    minioRegion: 'MinIO 区域',

    // Messages
    createSuccess: '任务创建成功',
    createFailed: '任务创建失败',
    updateSuccess: '任务更新成功',
    updateFailed: '任务更新失败',
    deleteSuccess: '任务删除成功',
    deleteFailed: '任务删除失败',
    runSuccess: '任务运行成功',
    runFailed: '任务运行失败',
    toggleSuccess: '任务状态切换成功',
    toggleFailed: '任务状态切换失败',
    deleteConfirm: '确定要删除这个任务吗？此操作不可撤销。',

    // Validation
    nameRequired: '请输入任务名称',
    typeRequired: '请选择任务类型',
    scheduleRequired: '请输入执行计划',
    scheduleInvalid: '执行计划格式无效，请使用 Cron 表达式',
    hostRequired: '请输入主机地址',
    portRequired: '请输入端口号',
    usernameRequired: '请输入用户名',
    passwordRequired: '请输入密码',
    databaseRequired: '请输入数据库名称',
    shareRequired: '请输入共享名称',
    pathRequired: '请输入路径',
    endpointRequired: '请输入端点地址',
    accessKeyRequired: '请输入访问密钥',
    secretKeyRequired: '请输入密钥',
    bucketRequired: '请输入存储桶名称',

    // Filters
    filterByType: '按类型筛选',
    filterByStatus: '按状态筛选',
    searchPlaceholder: '搜索任务名称...',
    allTypes: '所有类型',
    allStatuses: '所有状态',

    // Actions
    viewDetails: '查看详情',
    editDetails: '编辑详情',
    runNow: '立即运行',
    enable: '启用',
    disable: '禁用',
    duplicate: '复制',
    export: '导出',

    // Schedule helpers
    scheduleHelp: 'Cron 表达式格式：分 时 日 月 周',
    scheduleExamples: '示例：',
    scheduleDaily: '0 2 * * * (每天凌晨2点)',
    scheduleWeekly: '0 2 * * 0 (每周日凌晨2点)',
    scheduleMonthly: '0 2 1 * * (每月1号凌晨2点)',

    // Empty states
    noTasks: '暂无备份任务',
    noTasksDesc: '创建您的第一个备份任务开始使用',
    noFilterResults: '没有找到匹配的任务',
    noFilterResultsDesc: '尝试调整筛选条件或搜索关键词',

    // Additional fields
    subtitle: '管理您的 {total} 个备份任务',
    basicInfo: '基本信息',
    taskDetails: '任务详情',
    testConnection: '测试连接',
    connectionSuccess: '连接成功',
    connectionFailed: '连接失败',
    connectionTestSuccess: '连接测试成功',
    connectionTestFailed: '连接测试失败',
    connectionTestError: '连接测试出错',
    fillRequiredFields: '请填写必填字段',
    fillConnectionFields: '请先填写连接信息',
    showSensitive: '显示敏感信息',
    hideSensitive: '隐藏敏感信息',

    // Connection settings
    basicConnection: '基本连接',
    advancedOptions: '高级选项',
    sslConfiguration: 'SSL 配置',
    connectionSettings: '连接设置',
    enableSSL: '启用 SSL',
    sslCA: 'SSL CA 证书',
    sslCert: 'SSL 客户端证书',
    sslKey: 'SSL 客户端密钥',
    sslRejectUnauthorized: '拒绝未授权连接',
    sslCAPlaceholder: '输入 CA 证书内容...',
    sslCertPlaceholder: '输入客户端证书内容...',
    sslKeyPlaceholder: '输入客户端密钥内容...',
    connectionLimit: '连接池大小',
    connectionTimeout: '连接超时',

    // Table selection
    tableSelection: '表选择',
    loadTables: '加载表列表',
    selectAll: '全选',
    clearAll: '清空',
    noTablesFound: '未找到表',
    tablesLoaded: '已加载 {count} 个表',
    tablesLoadFailed: '加载表列表失败',

    // Sync options
    syncOptions: '同步选项',
    syncMode: '同步模式',
    selectSyncMode: '选择同步模式',
    forceMode: '强制模式',
    mergeMode: '合并模式',
    forceModeHelp: '强制模式会删除目标表并重新创建，完全覆盖数据',
    mergeModeHelp: '合并模式会保留目标表结构，只同步数据差异',
    batchSize: '批处理大小',
    batchSizeHelp: '每次处理的记录数量，影响内存使用和性能',
    dropTables: '删除目标表',
    dropTablesHelp: '同步前删除目标数据库中的表',
    truncateTables: '清空目标表',
    truncateTablesHelp: '同步前清空目标表数据',
    continueOnError: '遇错继续',
    continueOnErrorHelp: '遇到错误时继续处理其他表',

    // S3 sync options
    objectPrefix: '对象前缀',
    objectPrefixPlaceholder: '例如：backups/',
    objectPrefixHelp: '只同步指定前缀的对象',
    forceOverwrite: '强制覆盖',
    forceOverwriteHelp: '覆盖目标中已存在的文件',
    deleteExtraFiles: '删除多余文件',
    deleteExtraFilesHelp: '删除目标中源不存在的文件',
    delayMs: '延迟时间',
    delayMsHelp: '每个文件传输后的延迟时间',
    verboseLogging: '详细日志',
    verboseLoggingHelp: '记录详细的传输日志',

    // General options
    generalOptions: '通用选项',
    retryCount: '重试次数',
    retryCountHelp: '失败时的重试次数',
    retryDelay: '重试延迟',
    retryDelayHelp: '重试之间的延迟时间',
    taskTimeout: '任务超时',
    taskTimeoutHelp: '任务执行的最大时间限制',

    // MinIO specific
    minioPrefix: 'MinIO 前缀',
    minioPrefixHelp: '可选的对象前缀，用于组织备份文件',
    minioSSL: '使用 SSL',

    // Common
    enabled: '已启用',
    disabled: '已禁用',
    never: '从未'
  },

  // Time related
  time: {
    justNow: '刚刚',
    minutesAgo: '分钟前',
    hoursAgo: '小时前',
    daysAgo: '天前',
    weeksAgo: '周前',
    monthsAgo: '月前',
    yearsAgo: '年前',
    seconds: '秒',
    minutes: '分钟',
    hours: '小时',
    days: '天',
    weeks: '周',
    months: '月',
    years: '年'
  },

  storages: {
    title: '存储库管理',
    create: '创建存储库',
    edit: '编辑存储库',
    delete: '删除存储库',
    test: '测试连接',
    testAll: '批量测试',
    name: '存储库名称',
    type: '存储库类型',
    config: '配置信息',
    status: '连接状态',
    connected: '连接正常',
    disconnected: '连接失败',
    testing: '测试中...',
    mysql: 'MySQL数据库',
    minio: 'MinIO对象存储',
    s3: 'S3对象存储',
    smb: 'SMB网络共享',
    host: '主机地址',
    port: '端口',
    username: '用户名',
    password: '密码',
    database: '数据库名',
    endpoint: '端点地址',
    accessKey: 'Access Key',
    secretKey: 'Secret Key',
    bucket: '存储桶',
    useSSL: '使用SSL',
    sharePath: '共享路径',
    domain: '域',
    createSuccess: '存储库创建成功',
    updateSuccess: '存储库更新成功',
    deleteSuccess: '存储库删除成功',
    testSuccess: '连接测试成功',
    testFailed: '连接测试失败',
    deleteConfirm: '确定要删除此存储库吗？',
    deleteWarning: '删除后将无法恢复，请谨慎操作。'
  }
}
